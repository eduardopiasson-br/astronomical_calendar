    ...
    <section class="section-crud">
        ...
        <div class="col-md-6 div-form">
            ...
            <form id="missions" name="Cadastro" action="../cadastros/missions.php" method="POST">
                ...
                <textarea class="col-md-12" type="text" placeholder="Descreva detalhes da missão" name="MIS_DESCRIPTION"><?php echo $DATA_MIS['MIS_DESCRIPTION'] ?></textarea><br>

                <label>Data que ocorrerá </label>
                <input type="date" placeholder="Informe a data que acontecerá a Missão" name="MIS_DATE" value="<?php echo $DATA_MIS['MIS_DATE'] ?>"><br><br>

                <p>Caso a missão esteja associada a algum endereço cósmico, selecione-os nas opções abaixo </p>

                <!-- Combobox das galáxias -->
                <select class="select-form col-md-6 col-sm-12 col-12" name="MIG_GAL_CODE" id="MIG_GAL_CODE" form="missions">
                    <option value="0">Selecione uma Galáxia</option>
                    <?php while ($galaxys = mysqli_fetch_assoc($sql_info_galaxys)) : ?>
                        <option <? if($DATA_MIG['MIG_GAL_CODE']==$galaxys['GAL_CODE']) { ?> selected
                ...
                <select class="select-form col-md-6 col-sm-12 col-12" name="MSY_SYS_CODE" id="MSY_SYS_CODE" form="missions">
                    <option value="0">Selecione um Sistema</option>
                    <?php while ($systems = mysqli_fetch_assoc($sql_info_systems)) : ?>
                        <option <? if($DATA_MSY['MSY_SYS_CODE']==$systems['SYS_CODE']) { ?> selected
                ...
                <select class="select-form col-md-6 col-sm-12 col-12" name="MST_STA_CODE" id="MST_STA_CODE" form="missions">
                    <option value="0">Selecione uma Estrela</option>
                    <?php while ($stars = mysqli_fetch_assoc($sql_info_stars)) : ?>
                        <option <? if($DATA_MST['MST_STA_CODE']==$stars['STA_CODE']) { ?> selected
                ...
                <select class="select-form col-md-6 col-sm-12 col-12" name="MIP_PLA_CODE" id="MIP_PLA_CODE" form="missions">
                    <option value="0">Selecione um Planeta</option>
                    <?php while ($planets = mysqli_fetch_assoc($sql_info_planets)) : ?>
                        <option <? if($DATA_MIP['MIP_PLA_CODE']==$planets['PLA_CODE']) { ?> selected
                ...
                <select class="select-form col-md-6 col-sm-12 col-12" name="MSA_SAT_CODE" id="MSA_SAT_CODE" form="missions">
                    <option value="0">Selecione um Satélite Natural</option>
                    <?php while ($satellites = mysqli_fetch_assoc($sql_info_satellites)) : ?>
                        <option <? if($DATA_MSA['MSA_SAT_CODE']==$satellites['SAT_CODE']) { ?> selected
                ...
                <select class="select-form col-md-6 col-sm-12 col-12" name="MIC_COM_CODE" id="MIC_COM_CODE" form="missions">
                    <option value="0">Selecione um Cometa</option>
                    <?php while ($comets = mysqli_fetch_assoc($sql_info_comets)) : ?>
                        <option <? if($DATA_MIC['MIC_COM_CODE']==$comets['COM_CODE']) { ?> selected
