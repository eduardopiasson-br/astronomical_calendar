...
<body>
    <?php
        ...
        $phenomena = mysqli_fetch_assoc($sql_info_obj);
        ...
        <div class="col-md-6 div-form">
            ...
            <form id="phenomena" name="Cadastro" action="../cadastros/phenomena.php" method="POST">
                ...
                <? } ?>
                    ...
                    <select class="select-form" name="PHG_GAL_CODE" id="PHG_GAL_CODE" form="phenomena">
                        <option value="0">Selecione uma Galáxia</option>
                        <?php while ($galaxys = mysqli_fetch_assoc($sql_info_galaxys)) : ?>
                            <option <? if($DATA_PHG['PHG_GAL_CODE'] == $galaxys['GAL_CODE']) { ?> selected <? } ?> title="<?php echo $galaxys['GEN_NAME'] ?>" value="<?php echo $galaxys['GAL_CODE'] ?>"><?php echo $galaxys['GAL_CODE'] ?> - <?php echo $galaxys['GEN_NAME'] ?></option>                    
                        <?php endwhile; ?>
                    </select>

                    <!-- Combobox dos sistemas -->
                    <select class="select-form" name="PHS_SYS_CODE" id="PHS_SYS_CODE" form="phenomena">
                        <option value="0">Selecione um Sistema</option>
                        <?php while ($systems = mysqli_fetch_assoc($sql_info_systems)) : ?>
                            <option <? if($DATA_PHS['PHS_SYS_CODE'] == $systems['SYS_CODE']) { ?> selected <? } ?> title="<?php echo $systems['GEN_NAME'] ?>" value="<?php echo $systems['SYS_CODE'] ?>"><?php echo $systems['SYS_CODE'] ?> - <?php echo $systems['GEN_NAME'] ?></option>                    
                        <?php endwhile; ?>
                    </select>

                    <!-- Combobox das estrelas -->
                    <select class="select-form" name="PST_STA_CODE" id="PST_STA_CODE" form="phenomena">
                        <option value="0">Selecione uma Estrela</option>
                        <?php while ($stars = mysqli_fetch_assoc($sql_info_stars)) : ?>
                            <option <? if($DATA_PST['PST_STA_CODE'] == $stars['STA_CODE']) { ?> selected <? } ?> title="<?php echo $stars['GEN_NAME'] ?>" value="<?php echo $stars['STA_CODE'] ?>"><?php echo $stars['STA_CODE'] ?> - <?php echo $stars['GEN_NAME'] ?></option>                    
                        <?php endwhile; ?>
                    </select>

                    <!-- Combobox dos planetas -->
                    <select class="select-form" name="PHP_PLA_CODE" id="PHP_PLA_CODE" form="phenomena">
                        <option value="0">Selecione um Planeta</option>
                        <?php while ($planets = mysqli_fetch_assoc($sql_info_planets)) : ?>
                            <option <? if($DATA_PHP['PHP_PLA_CODE'] == $planets['PLA_CODE']) { ?> selected <? } ?> title="<?php echo $planets['GEN_NAME'] ?>" value="<?php echo $planets['PLA_CODE'] ?>"><?php echo $planets['PLA_CODE'] ?> - <?php echo $planets['GEN_NAME'] ?></option>                    
                        <?php endwhile; ?>
                    </select>

                    <!-- Combobox dos satélites naturais -->
                    <select class="select-form" name="PSA_SAT_CODE" id="PSA_SAT_CODE" form="phenomena">
                        <option value="0">Selecione um Satélite Natural</option>
                        <?php while ($satellites = mysqli_fetch_assoc($sql_info_satellites)) : ?>
                            <option <? if($DATA_PSA['PSA_SAT_CODE'] == $satellites['SAT_CODE']) { ?> selected <? } ?> title="<?php echo $satellites['GEN_NAME'] ?>" value="<?php echo $satellites['SAT_CODE'] ?>"><?php echo $satellites['SAT_CODE'] ?> - <?php echo $satellites['GEN_NAME'] ?></option>                    
                        <?php endwhile; ?>
                    </select>

                    <!-- Combobox dos cometas -->
                    <select class="select-form" name="PHC_COM_CODE" id="PHC_COM_CODE" form="phenomena">
                        <option value="0">Selecione um Cometa</option>
                        <?php while ($comets = mysqli_fetch_assoc($sql_info_comets)) : ?>
                            <option <? if($DATA_PHC['PHC_COM_CODE'] == $comets['COM_CODE']) { ?> selected <? } ?> title="<?php echo $comets['GEN_NAME'] ?>" value="<?php echo $comets['COM_CODE'] ?>"><?php echo $comets['COM_CODE'] ?> - <?php echo $comets['GEN_NAME'] ?></option>                    
                        <?php endwhile; ?>
                    </select>
