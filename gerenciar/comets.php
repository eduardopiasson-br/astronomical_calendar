    ...
    <section class="section-crud">
        ...
        <div class="col-md-6 div-form">
            ...
            <form id="comets" name="Cadastro" action="../cadastros/comets.php" method="POST">
                ...
                <label>Nome </label>
                <input type="text" name="GEN_NAME" placeholder="Informe o nome do Cometa" value="<?php echo $DATA_GEN['GEN_NAME'] ?>"><br>

                <label>Descrição </label>
                <textarea class="col-md-12" type="text" placeholder="Descreva detalhes do Cometa" name="GEN_DESCRIPTION"><?php echo $DATA_GEN['GEN_DESCRIPTION'] ?></textarea><br>

                <label>Distância da Terra (em UA)</label>
                <input type="number" placeholder="Informe a distância do Cometa com relação ao Planeta Terra" name="GEN_DISTANCE" value="<?php echo $DATA_GEN['GEN_DISTANCE'] ?>"><br>

                <label>Massa (em KG)</label>
                <input type="number" placeholder="Informe a massa do Cometa" name="GEN_MASS" value="<?php echo $DATA_GEN['GEN_MASS'] ?>"><br>

                <label>Data da Descoberta</label>
                <input type="date" name="COM_DISCOVERY" value="<?php echo $DATA_OBJ['COM_DISCOVERY'] ?>"><br>

                <!-- Section com dados associados -->
                <select class="select-form" name="COM_GAL_CODE" id="COM_GAL_CODE" form="comets" required>
                    ...
                    <?php while ($associated = mysqli_fetch_assoc($sql_info_associated)) : ?>
                        <option <? if($DATA_OBJ['COM_GAL_CODE']==$associated['GAL_CODE']) { ?> selected
                            <? } ?> title="<?php echo $associated['GEN_NAME'] ?>" value="<?php echo $associated['GAL_CODE'] ?>"><?php echo $associated['GAL_CODE'] ?> - <?php echo $associated['GEN_NAME'] ?></option>
                    <?php endwhile; ?>
