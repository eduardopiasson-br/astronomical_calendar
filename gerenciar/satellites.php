    ...
    <section class="section-crud">
        ...
        <div class="col-md-6 div-form">
            ...
            <form id="satellites" name="Cadastro" action="../cadastros/satellites.php" method="POST">
                ...
                <label>Nome </label>
                <input type="text" name="GEN_NAME" placeholder="Informe o nome do Satélite" value="<?php echo $DATA_GEN['GEN_NAME'] ?>"><br>

                <label>Descrição </label>
                <textarea class="col-md-12" type="text" placeholder="Descreva detalhes do Satélite" name="GEN_DESCRIPTION"><?php echo $DATA_GEN['GEN_DESCRIPTION'] ?></textarea><br>

                <label>Distância da Terra (em AL)</label>
                <input type="NUMBER" placeholder="Informe a distância do Satélite com relação ao Planeta Terra" name="GEN_DISTANCE" value="<?php echo $DATA_GEN['GEN_DISTANCE'] ?>"><br>

                <label>Massa (em Kg)</label>
                <input type="NUMBER" placeholder="Informe a massa do Satélite" name="GEN_MASS" value="<?php echo $DATA_GEN['GEN_MASS'] ?>"><br>
                ...
                <select class="select-form" name="SAT_PLA_CODE" id="SAT_PLA_CODE" form="satellites" required>
                    ...
                    <?php while ($associated = mysqli_fetch_assoc($sql_info_associated)) : ?>
                        <option <? if($DATA_OBJ['SAT_PLA_CODE']==$associated['PLA_CODE']) { ?> selected
                            <? } ?> title="<?php echo $associated['GEN_NAME'] ?>" value="<?php echo $associated['PLA_CODE'] ?>"><?php echo $associated['PLA_CODE'] ?> - <?php echo $associated['GEN_NAME'] ?></option>
                    <?php endwhile; ?>
                </select><br>
